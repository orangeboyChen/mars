...
fn read_records(path: &str) -> Result<Vec<Vec<u8>>, String> {
...
fn count_blocks(bytes: &[u8]) -> Result<usize, String> {
    let mut count = 0;
    let mut offset = 0;
...
fn decode(opts: &Opts) -> Result<(), String> {
...
fn decode_records(data: &[u8], privkey: &[u8; 32]) -> Result<Vec<u8>, String> {
